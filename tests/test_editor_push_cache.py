...
def mock_services(monkeypatch):
    """Patch module-level globals in editor_server so the app works without real Jama."""
    ...
    monkeypatch.setattr(srv, "editor_cache", mock_cache)
    monkeypatch.setattr(srv, "schema_sync", mock_schema)
    monkeypatch.setattr(srv, "attachment_mgr", mock_att)
...
class TestRewriteImageUrls:
    def test_rewrites_jama_url(self):
        from jama_editor.editor_server import _rewrite_image_urls, EDITOR_PORT

        html = '<img src="https://enphase.jamacloud.com/rest/v1/attachments/12345/file">'
        result = _rewrite_image_urls(html)
        assert f"http://localhost:{EDITOR_PORT}/api/proxy/image/12345" in result
    ...
    def test_multiple_images(self):
        ...
        result = _rewrite_image_urls(html)
        assert f"http://localhost:{EDITOR_PORT}/api/proxy/image/111" in result
        assert f"http://localhost:{EDITOR_PORT}/api/proxy/image/222" in result
        assert "jamacloud.com" not in result
...
class TestProxyImageEndpoint:
    ...
    async def test_proxy_image_404(self, client, mock_services):
        """GET /api/proxy/image/{id} should return error for missing attachment."""
        mock_api = mock_services["api"]
