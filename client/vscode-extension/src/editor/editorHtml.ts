...
export function getEditorHtml(
  ...
  // URI for the VS Code webview-ui-toolkit bundle
  const toolkitUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "out", "webview", "toolkit.js")
  );
  ...
  const transitionOptions = transitions
    .map(
      (t) =>
        `<vscode-option value="${escapeHtml(t.id)}">${escapeHtml(t.action)}</vscode-option>`
    )
    .join("\n");
  ...
  const fieldRows = fieldDefs
    ...
    .map((fd) => renderFieldInput(fd, fields[fd.name], pickLists));
  const fieldCount = fieldRows.length;
  const dynamicFieldsContent = fieldCount > 0 ? fieldRows.join("\n") : "";
  ...
  const versionsPanelHtml = versions.length > 0
    ? `<p style="color:var(--vscode-descriptionForeground);font-size:12px;margin-bottom:8px;">
       ...
       <ul style="list-style:none;padding:0;margin:0;">
         ${versions.slice().sort((a, b) => b.version_num - a.version_num).map(v =>
           `<li style="padding:6px 0;border-bottom:1px solid var(--vscode-panel-border);font-size:12px;">
              <b>v${v.version_num}</b>${v.modified_by ? ` — ${escapeHtml(String(v.modified_by))}` : ""}${v.created_date ? ` <span style="color:var(--vscode-descriptionForeground)">(${escapeHtml(v.created_date)})</span>` : ""}
            </li>`
         ).join("")}
       </ul>`
    : `<p style="color:var(--vscode-descriptionForeground)">No version history available.</p>`;
...
<div class="page-content">
  <!-- Toolbar -->
  <div class="toolbar">
    <vscode-badge>${docKey}</vscode-badge>
    ...
    <span class="edit-actions">
      ...
      <vscode-button id="btnUnlock" appearance="secondary" style="display:none">🔓 Unlock</vscode-button>
  ...
  <vscode-panels activeid="tab-desc" aria-label="Item sections">
    ...
    <vscode-panel-tab id="tab-fields">Fields (${fieldCount})</vscode-panel-tab>
    <vscode-panel-tab id="tab-comments">Comments (${commentCount})</vscode-panel-tab>
    <vscode-panel-tab id="tab-attachments">Attachments</vscode-panel-tab>
    <vscode-panel-tab id="tab-versions">Versions (${versionCount})</vscode-panel-tab>
    ...
    <vscode-panel-view id="view-fields">
      ${fieldCount > 0
        ...
        : "<p style='color:var(--vscode-descriptionForeground)'>No additional fields for this item type.</p>"}
    </vscode-panel-view>
...
function renderFieldInput(
  ...
  switch (fd.fieldType) {
    ...
    case "RICHTEXT":
    case "DOCUMENT":
      inputHtml = `<vscode-text-area id="${inputId}" data-field-name="${fieldName}" value="${escapeAttr(strVal)}" rows="5"${readOnlyAttr}${trackAttr}></vscode-text-area>`;
      break;

    case "TEXT":
    default: {
      const maxLen = fd.maxLength ? ` maxlength="${fd.maxLength}"` : "";
      inputHtml = `<vscode-text-field id="${inputId}" data-field-name="${fieldName}" value="${escapeAttr(strVal)}"${readOnlyAttr}${maxLen}${trackAttr}></vscode-text-field>`;
      break;
    }
  }
