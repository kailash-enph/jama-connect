...
function getToolkitUri(webview: vscode.Webview, extensionUri: vscode.Uri): vscode.Uri {
...
function getCspMeta(toolkitSrc?: string): string {
  const base = getEditorBaseUrl();
  const toolkitAllow = toolkitSrc ? ` ${toolkitSrc}` : "";
  return `<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'${toolkitAllow}; img-src https: data: ${base};">`;
}
...
function statusBadge(status: string): string {
  ...
  return `<span class="badge" style="background:${c.bg};color:${c.text};display:inline-block;padding:2px 10px;border-radius:12px;font-size:12px;font-weight:600;">${status}</span>`;
}
...
export function getTestPlanDetailHtml(
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8" />${getCspMeta(webview.cspSource)}<script src="${toolkitUri}"></script><style>${SHARED_CSS}</style></head>
<body>
  ${toolbarHtml()}
  <div style="display:flex;align-items:center;gap:10px;">
    ...
    <vscode-tag>${escapeHtml(plan.status)}</vscode-tag>
  </div>
...
<body>
  ...
  <div style="display:flex;align-items:center;gap:10px;">
    ...
    <vscode-tag>${escapeHtml(cycle.status)}</vscode-tag>
  </div>
...
<body>
  ...
  <div style="display:flex;align-items:center;gap:10px;">
    ...
    <div class="readonly-field"><vscode-tag>${escapeHtml(run.status)}</vscode-tag></div>
    ${steps.length === 0 ? `<div class="editable-field"><select class="field-select" id="f_testRunStatus" data-field="testRunStatus">${statusOptions}</select></div>` : `<div class="editable-field"><span style="font-size:12px;color:var(--vscode-descriptionForeground);"><vscode-tag>${escapeHtml(run.status)}</vscode-tag> (derived from steps)</span></div>`}
  </div>
