...
Status: ${s.status}
Assigned: ${s.assigned_to??"unassigned"}`,a.iconPath=this.getStatusIcon(s.status),a.command={command:"jamaEditor.openTestDetail",title:"Open Test Run",arguments:["run",s]},a})}catch(t){let s=t instanceof Error?t.message:String(t);return f.window.showErrorMessage(`Failed to load test runs: ${s}`),[]}}getStatusIcon(e){let t=e.toUpperCase();return t==="PASSED"?new f.ThemeIcon("testing-passed-icon",new f.ThemeColor("testing.iconPassed")):t==="FAILED"?new f.ThemeIcon("testing-failed-icon",new f.ThemeColor("testing.iconFailed")):t==="BLOCKED"?new f.ThemeIcon("testing-skipped-icon",new f.ThemeColor("testing.iconSkipped")):t==="NOT_RUN"||t==="NOT RUN"?new f.ThemeIcon("testing-unset-icon"):t==="IN_PROGRESS"||t==="INPROGRESS"?new f.ThemeIcon("testing-queued-icon",new f.ThemeColor("testing.iconQueued")):new f.ThemeIcon("circle-outline")}},A=class extends f.TreeItem{testPlanId;testCycleId;testRunId;testPlanData;testCycleData;testRunData};var u=T(require("vscode"));var ce=T(require("vscode"));function we(i,e,t){let{item:s,fields:a,comments:n,transitions:o}=t,r=t.fieldDefinitions??[],d=t.pickListOptions??{},c=t.versions??[],p=t.drafts??[],g=He(),m=`http://localhost:${$().port}/editor`,v=i.asWebviewUri(ce.Uri.joinPath(e,"out","webview","tiptap.js")),w=i.asWebviewUri(ce.Uri.joinPath(e,"out","webview","toolkit.js")),b=_(s.name),x=s.description??"",h=_(s.document_key),y=s.version??s.current_version??0,D=_(s.global_id??""),O=s.modified_date??"",R=o.map(k=>`<vscode-option value="${_(k.id)}">${_(k.action)}</vscode-option>`).join(`
`),K=n.map(k=>{let oe=k.createdBy?`${_(k.createdBy.firstName??"")} ${_(k.createdBy.lastName??"")}`:"Unknown",Je=k.createdDate??"",Ue=k.body?.text??"";return`<div class="comment">
        <div class="comment-header"><strong>${oe}</strong> <span class="date">${_(Je)}</span></div>
        <div class="comment-body">${Ue}</div>
      </div>`}).join(`
`),ie=new Set(["name","description"]),F=r.filter(k=>!ie.has(k.name)).map(k=>We(k,a[k.name],d)),re=F.length,De=re>0?F.join(`
`):"",je=n.length,Re=c.length,Me=Ge(c,p),Ae=c.length>0?`<p style="color:var(--vscode-descriptionForeground);font-size:12px;margin-bottom:8px;">
       ...
       <ul style="list-style:none;padding:0;margin:0;">
         ${c.slice().sort((k,oe)=>oe.version_num-k.version_num).map(k=>`<li style="padding:6px 0;border-bottom:1px solid var(--vscode-panel-border);font-size:12px;">
              <b>v${k.version_num}</b>${k.modified_by?` \u2014 ${_(String(k.modified_by))}`:""}${k.created_date?` <span style="color:var(--vscode-descriptionForeground)">(${_(k.created_date)})</span>`:""}
            </li>`).join("")}
...
<div class="page-content">
  ...
  <div class="section" style="margin-top:20px;">
    ...
    <div class="section-body hidden">
      <pre class="fields-json">${_(Le)}</pre>
    </div>
  </div>
...
</html>`}function He(){let i="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",e="";for(let t=0;t<32;t++)e+=i.charAt(Math.floor(Math.random()*i.length));return e}function _(i){return i==null?"":String(i).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function L(i){return i==null?"":String(i).replace(/"/g,"&quot;").replace(/&/g,"&amp;")}function We(i,e,t){let s=_(i.label),a=L(i.name),n=i.required?' <span class="required">*</span>':"",o=i.readOnly?" field-readonly":"",r=i.readOnly?" disabled":"",d=i.readOnly?"":` data-track-field="${a}"`,c=e!=null?String(e):"",p=`dynField_${i.name}`,g;switch(i.fieldType){case"PICK_LIST":{let v=(i.pickList!=null?t[i.pickList]??[]:[]).filter(w=>w.active!==!1).map(w=>{let b=L(String(w.id)),x=_(w.name),h=String(w.id)===c?" selected":"";return`<option value="${b}"${h}>${x}</option>`}).join(`
`);g=`<select id="${p}" data-field-name="${a}"${r} data-track-field="${a}">
        <option value="">(none)</option>
        ${v}
      </select>`;break}case"MULTI_SELECT":{let m=i.pickList!=null?t[i.pickList]??[]:[],v=Array.isArray(e)?e.map(String):[],w=m.filter(b=>b.active!==!1).map(b=>{let x=L(String(b.id)),h=_(b.name),y=v.includes(String(b.id))?" selected":"";return`<option value="${x}"${y}>${h}</option>`}).join(`
`);g=`<select id="${p}" data-field-name="${a}" multiple${r} data-track-field="${a}">
        ${w}
      </select>`;break}case"BOOLEAN":g=`<input type="checkbox" id="${p}" data-field-name="${a}"${e?" checked":""}${r} data-track-field="${a}" />`;break;case"INTEGER":case"FLOAT":g=`<input type="number" id="${p}" data-field-name="${a}" value="${L(c)}"${r}${d}${i.fieldType==="INTEGER"?' step="1"':' step="any"'} />`;break;case"DATE":g=`<input type="date" id="${p}" data-field-name="${a}" value="${L(c)}"${r}${d} />`;break;case"URL":g=`<input type="url" id="${p}" data-field-name="${a}" value="${L(c)}"${r}${d} />`;break;case"RICHTEXT":case"DOCUMENT":g=`<vscode-text-area id="${p}" data-field-name="${a}" value="${L(c)}" rows="5"${r}${d}></vscode-text-area>`;break;case"TEXT":default:{let m=i.maxLength?` maxlength="${i.maxLength}"`:"";g=`<vscode-text-field id="${p}" data-field-name="${a}" value="${L(c)}"${r}${m}${d}></vscode-text-field>`;break}}return`<div class="form-group">
    <label>${s}${n}</label>
    <div class="${o}">${g}</div>
  </div>`}function Ge(i,e){if(i.length===0&&e.length===0)return"";let t=i.slice().sort((a,n)=>n.version_num-a.version_num).map(a=>{let n=a.modified_by?` \u2014 ${_(String(a.modified_by))}`:"",o=a.created_date?` (${_(a.created_date)})`:"";return`<option value="v:${a.version_num}">v${a.version_num}${n}${o}</option>`}).join(`
`),s=e.slice().sort((a,n)=>n.draft_version-a.draft_version).map(a=>{let n=a.created_at?new Date(a.created_at*1e3).toLocaleString():"",o=a.is_autosave?" (auto)":"";return`<option value="d:${a.draft_version}">Draft #${a.draft_version}${o} \u2014 ${_(n)}</option>`}).join(`
`);return`<select id="versionDraftSelect" style="max-width:200px">
  ...
  </select>`}var Ke=5e3,Ze=/https?:\/\/[^"'\s]*?\/(?:rest\/v1\/(?:attachments|files)\/(\d+)(?:\/file)?|attachment\/(\d+)\/[^"'\s]*)/gi;function le(){return`http://localhost:${$().port}/editor`}function xe(i){let e=le();return i.replace(Ze,(t,s,a)=>`${e}/api/proxy/image/${s||a}`)}var ne=class i{static viewType="jamaEditor.itemEditor";panels=new Map;autosaveTimers=new Map;serverVersions=new Map;api;editorApi;extensionUri;constructor(e,t){this.api=e,this.editorApi=new z,this.extensionUri=t}get openTabCount(){return this.panels.size}getEditorApi(){return this.editorApi}async openItem(e,t,s,a){let n=this.panels.get(e);if(n){n.reveal(u.ViewColumn.One);return}let o=u.window.createWebviewPanel(i.viewType,`${a} \u2014 ${s}`,u.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[u.Uri.joinPath(this.extensionUri,"media"),u.Uri.joinPath(this.extensionUri,"out")]});this.panels.set(e,o),o.onDidDispose(async()=>{this.stopAutosave(e),this.panels.delete(e),this.serverVersions.delete(e);try{await this.editorApi.releaseLock(e)}catch{}}),o.webview.onDidReceiveMessage(async r=>{try{await this.handleWebviewMessage(o,e,t,r)}catch(d){let c=d instanceof Error?d.message:String(d);console.error(`[jama-editor] Error handling message '${r?.type}': ${c}`),u.window.showErrorMessage(`Jama Editor error: ${c}`)}}),await u.window.withProgress({location:u.ProgressLocation.Notification,title:"Syncing item from Jama\u2026",cancellable:!1},()=>this.loadItem(o,e,t)),o.webview.postMessage({type:"lockState",locked:!1,editable:!1})}dispose(){for(let[e]of this.autosaveTimers)this.stopAutosave(e);for(let[e,t]of this.panels)this.editorApi.releaseLock(e).catch(()=>{}),t.dispose();this.panels.clear(),this.serverVersions.clear()}startAutosave(e,t){this.stopAutosave(e);let s=setInterval(()=>{t.webview.postMessage({type:"requestAutosave"})},Ke);this.autosaveTimers.set(e,s)}stopAutosave(e){let t=this.autosaveTimers.get(e);t&&(clearInterval(t),this.autosaveTimers.delete(e))}async loadItem(e,t,s){try{let a,n=!0;try{a=await this.api.getItem(t,!0)}catch{n=!1,a=await this.api.getItem(t)}let[o,r]=await Promise.all([this.api.getItemComments(t).catch(()=>[]),this.api.getWorkflowTransitions(t).catch(()=>[])]);n||u.window.showWarningMessage("Could not sync from Jama \u2014 showing cached data."),this.serverVersions.set(t,a.version??0);let d=a.fields_json?JSON.parse(a.fields_json):{},c=[],p={};try{let h=a.item_type;if(h){c=(await this.editorApi.getFieldDefinitions(h)).fields??[];let D=c.filter(R=>R.pickList!=null).map(R=>R.pickList),O=[...new Set(D)];if(O.length>0){let R=await Promise.all(O.map(K=>this.editorApi.getPickListOptions(K).catch(()=>[])));O.forEach((K,ie)=>{let F=R[ie];p[K]=Array.isArray(F)?F:F?.options??[]})}}}catch{}let g=[];try{g=await this.api.getItemVersions(t)??[]}catch{}let m=[],v=d,w=a.description??"",b=!1;try{m=(await this.editorApi.getDrafts(t)).drafts??[];let y=await this.editorApi.getLatestDraft(t);y&&y.fields_json&&(v=JSON.parse(y.fields_json),w=y.description_html??w,b=!0)}catch{}let x=xe(w);e.webview.html=we(e.webview,this.extensionUri,{item:{...a,description:x},fields:v,comments:o,transitions:r,projectId:s,fieldDefinitions:c,pickListOptions:p,versions:g,drafts:m}),b&&e.webview.postMessage({type:"draftRestored"}),this.editorApi.syncAttachments(t).then(h=>{e.webview.postMessage({type:"attachmentsLoaded",attachments:h.attachments})}).catch(()=>{})}catch(a){let n=a instanceof Error?a.message:String(a);u.window.showErrorMessage(`Failed to load item ${t}: ${n}`),e.webview.html=`<html><body><h2>Error loading item</h2><pre>${n}</pre></body></html>`}}async handleWebviewMessage(e,t,s,a){switch(a.type!=="autosaveData"&&a.type!=="fieldChanged"&&console.log(`[jama-editor] Received message: ${a.type} for item ${t}`),a.type){case"autosaveData":{try{let n=this.serverVersions.get(t)??0;await this.editorApi.saveDraft(t,n,JSON.stringify(a.fields??{}),a.descriptionHtml??"",!0)}catch{}break}case"push":{try{let n=this.serverVersions.get(t)??0,o=await this.editorApi.pushToJama(t,a.fields??{},n);this.serverVersions.set(t,o.version),u.window.showInformationMessage(`Pushed to Jama \u2192 v${o.version}`),await this.loadItem(e,t,s),e.webview.postMessage({type:"pushComplete",success:!0,version:o.version}),u.commands.executeCommand("jamaEditor.refreshTree")}catch(n){let o=n instanceof Error?n.message:String(n);if(o.includes("version_conflict")){let r=await u.window.showWarningMessage("Item was modified on the server. Overwrite or reload?","Force Push","Reload Server Version");if(r==="Force Push")try{let d=await this.editorApi.pushToJama(t,a.fields??{});this.serverVersions.set(t,d.version),u.window.showInformationMessage(`Force-pushed \u2192 v${d.version}`),await this.loadItem(e,t,s),e.webview.postMessage({type:"pushComplete",success:!0,version:d.version}),u.commands.executeCommand("jamaEditor.refreshTree")}catch(d){let c=d instanceof Error?d.message:String(d);u.window.showErrorMessage(`Force push failed: ${c}`)}else r==="Reload Server Version"&&await this.loadItem(e,t,s)}else u.window.showErrorMessage(`Push failed: ${o}`),e.webview.postMessage({type:"pushComplete",success:!1,error:o})}break}case"undo":{try{let n=await this.editorApi.popUndo(t);e.webview.postMessage({type:"undoApply",fieldName:n.field_name,value:n.old_value})}catch{u.window.showInformationMessage("Nothing to undo.")}break}case"fieldChanged":{try{await this.editorApi.pushUndo(t,a.fieldName??"",a.oldValue??null,a.newValue??null)}catch{}break}case"revert":{try{await this.editorApi.clearDrafts(t)}catch{}await this.loadItem(e,t,s);break}case"edit":{if(this.autosaveTimers.size>0&&this.autosaveTimers.keys().next().value!==t){u.window.showWarningMessage("Another item is already being edited. Unlock it first before editing this one.");break}let n="";try{await this.editorApi.acquireLock(t),n="Lock acquired \u2014 editing enabled."}catch(o){let r=o instanceof Error?o.message:String(o);n=r.includes("workflow")?"Item is workflow-locked on Jama \u2014 editing locally.":`Could not lock on Jama \u2014 editing locally. (${r.slice(0,80)})`}this.startAutosave(t,e),e.webview.postMessage({type:"lockState",locked:!0,editable:!0}),u.window.showInformationMessage(n);break}case"unlock":{try{await this.editorApi.releaseLock(t),this.stopAutosave(t),e.webview.postMessage({type:"lockState",locked:!1,editable:!1}),u.window.showInformationMessage("Read-only mode \u2014 item unlocked.")}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Unlock failed: ${o}`)}break}case"transition":{try{let n=await u.window.showInputBox({prompt:"Transition comment (optional)",placeHolder:"Enter a comment for this workflow transition..."});await this.api.executeWorkflowTransition(t,a.transitionId??"",n??""),u.window.showInformationMessage("Workflow transition applied."),await this.loadItem(e,t,s),u.commands.executeCommand("jamaEditor.refreshTree")}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Transition failed: ${o}`)}break}case"addComment":{try{await this.api.addItemComment(t,a.text??""),await this.loadItem(e,t,s)}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Comment failed: ${o}`)}break}case"loadVersion":{try{let n=a.version??0;if(n<=0)break;let o=await this.api.getItemAtVersion(t,n);if(o){let r=o.fields_json?JSON.parse(o.fields_json):{};e.webview.postMessage({type:"versionData",version:n,fields:r,descriptionHtml:o.description_html??""}),e.webview.postMessage({type:"lockState",locked:!1,editable:!1}),u.window.showInformationMessage(`Viewing server version ${n} (read-only).`)}}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Failed to load version: ${o}`)}break}case"imageUpload":{try{let n=await u.window.showOpenDialog({canSelectFiles:!0,canSelectMany:!1,openLabel:"Upload Image",filters:{Images:["png","jpg","jpeg","gif","bmp","webp","svg"]}});if(!n||n.length===0)break;let o=n[0].fsPath,r=await this.editorApi.uploadAttachment(t,o,a.fileName??"");if(r?.attachmentId){let d=`${le()}/api/proxy/image/${r.attachmentId}`;e.webview.postMessage({type:"insertImage",src:d,alt:a.fileName??""})}}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Image upload failed: ${o}`)}break}case"loadDraft":{try{let n=a.draftVersion??0;if(n<=0)break;let o=await this.editorApi.getDraft(t,n);if(o&&o.fields_json){let r=JSON.parse(o.fields_json);for(let[d,c]of Object.entries(r))e.webview.postMessage({type:"undoApply",fieldName:d,value:c!=null?String(c):""});u.window.showInformationMessage(`Restored draft #${n}.`)}}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Failed to load draft: ${o}`)}break}case"syncAttachments":{try{let n=await this.editorApi.syncAttachments(t);e.webview.postMessage({type:"attachmentsLoaded",attachments:n.attachments})}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Failed to sync attachments: ${o}`)}break}case"uploadAttachment":{try{let n=await u.window.showOpenDialog({canSelectFiles:!0,canSelectMany:!1,openLabel:"Upload Attachment"});if(!n||n.length===0)break;let o=n[0].fsPath,r=o.split(/[\\/]/).pop()??"";await this.editorApi.uploadAttachment(t,o,r),u.window.showInformationMessage(`Uploaded "${r}".`);let d=await this.editorApi.syncAttachments(t);e.webview.postMessage({type:"attachmentsLoaded",attachments:d.attachments})}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Upload failed: ${o}`)}break}case"downloadAttachment":{try{let n=a.attachmentId??0;if(n<=0)break;let o=`${le()}/api/attachments/${n}/download`;await u.env.openExternal(u.Uri.parse(o))}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Download failed: ${o}`)}break}case"deleteAttachment":{u.window.showWarningMessage("Deleting attachments is not allowed from this app.");break}case"importImages":{try{let o=(await this.api.getItem(t,!0))?.description??"",r=xe(o);e.webview.postMessage({type:"updateDescription",html:r})}catch{u.window.showErrorMessage("Failed to import images.")}break}case"replaceAttachment":{try{let n=a.attachmentId??0;if(n<=0)break;let o=await u.window.showOpenDialog({canSelectFiles:!0,canSelectMany:!1,openLabel:"Replace Attachment"});if(!o||o.length===0)break;let r=o[0].fsPath,d=r.split(/[\\/]/).pop()??"";await this.editorApi.replaceAttachment(n,r,d),u.window.showInformationMessage(`Replaced with "${d}".`);let c=await this.editorApi.syncAttachments(t);e.webview.postMessage({type:"attachmentsLoaded",attachments:c.attachments})}catch(n){let o=n instanceof Error?n.message:String(n);u.window.showErrorMessage(`Replace failed: ${o}`)}break}}}};var Ee=T(require("vscode"));function pe(i,e){return i.asWebviewUri(Ee.Uri.joinPath(e,"out","webview","toolkit.js"))}var Ye=/https?:\/\/[^"'\s]*?\/(?:rest\/v1\/(?:attachments|files)\/(\d+)(?:\/file)?|attachment\/(\d+)\/[^"'\s]*)/gi;function _e(){return`http://localhost:${$().port}/editor`}function U(i){let e=_e();return i.replace(Ye,(t,s,a)=>`${e}/api/proxy/image/${s||a}`)}function me(i){let e=_e();return`<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'${i?` ${i}`:""}; img-src https: data: ${e};">`}var ke={PASSED:{bg:"#166534",text:"#bbf7d0"},FAILED:{bg:"#991b1b",text:"#fecaca"},BLOCKED:{bg:"#854d0e",text:"#fef08a"},INPROGRESS:{bg:"#1e40af",text:"#bfdbfe"},IN_PROGRESS:{bg:"#1e40af",text:"#bfdbfe"},NOT_RUN:{bg:"#374151",text:"#d1d5db"}};function H(i){let e=i.toUpperCase(),t=ke[e]??ke.NOT_RUN;return`<span class="badge" style="background:${t.bg};color:${t.text};display:inline-block;padding:2px 10px;border-radius:12px;font-size:12px;font-weight:600;">${i}</span>`}function E(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var ue=`
...
`;function ve(){return`<div class="toolbar">
  ...
  </div>`}function Te(i){if(i.length===0)return"";let e={PASSED:0,FAILED:0,BLOCKED:0,INPROGRESS:0,NOT_RUN:0};for(let t of i){let s=t.status?.toUpperCase()??"NOT_RUN";e[s]=(e[s]??0)+1}return $e(e,i.length,"Runs")}function $e(i,e,t){let s=r=>e>0?Math.round(r/e*100):0,a={PASSED:"#22c55e",FAILED:"#ef4444",BLOCKED:"#eab308",INPROGRESS:"#3b82f6",NOT_RUN:"#6b7280"},n=Object.entries(i).filter(([,r])=>r>0).map(([r,d])=>`<span class="step-count"><span class="dot" style="background:${a[r]??"#6b7280"};"></span>${d} ${r}</span>`).join(""),o=Object.entries(i).filter(([,r])=>r>0).map(([r,d])=>`<div class="seg" style="width:${s(d)}%;background:${a[r]??"#6b7280"};"></div>`).join("");return`
    <div class="summary-item" style="grid-column: 1 / -1;">
      <div class="summary-label">${t} Progress (${e})</div>
      <div class="progress-bar">${o}</div>
      <div class="step-summary">${n}</div>
    </div>`}function Qe(i,e,t){let s=[];s.push(`<div class="summary-item"><div class="summary-label">Plan ID</div><div class="summary-value">${i.id}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Project</div><div class="summary-value">${i.project_id}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Status</div><div class="summary-value">${H(i.status)}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Cycles</div><div class="summary-value">${e.length}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Total Runs</div><div class="summary-value">${t.length}</div></div>`);let a=Te(t);return`<div class="summary-card">
    <div style="font-size:13px;font-weight:600;color:var(--vscode-descriptionForeground);">SUMMARY</div>
    <div class="summary-grid">${s.join(`
`)}${a}</div>
  </div>`}function Ie(i,e,t,s,a){let n=pe(i,e),o=s.map(p=>`<tr style="cursor:pointer;" onclick="openCycle(${p.id})">
        <td>${E(p.name)}</td>
        <td>${H(p.status)}</td>
        <td>${p.start_date?new Date(p.start_date).toLocaleDateString():"\u2014"}</td>
        <td>${p.end_date?new Date(p.end_date).toLocaleDateString():"\u2014"}</td>
      </tr>`).join(""),r={};try{r=JSON.parse(t.fields_json||"{}")}catch{}let d=U(r.description??t.description??""),c=Qe(t,s,a??[]);return`<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8" />${me(i.cspSource)}<script src="${n}"></script><style>${ue}</style></head>
<body>
  ${ve()}
  ...
  <div class="section">
    <h2>Description</h2>
    <div class="readonly-field">${d?`<div class="desc">${d}</div>`:'<span style="color:var(--vscode-descriptionForeground);">\u2014</span>'}</div>
    <div class="editable-field"><textarea class="field-textarea" id="f_description" data-field="description">${E(typeof d=="string"?d.replace(/<[^>]*>/g,""):"")}</textarea></div>
  </div>

  <div class="section">
    <h2>Test Cycles (${s.length})</h2>
    ${s.length>0?`<table><thead><tr><th>Cycle</th><th>Status</th><th>Start</th><th>End</th></tr></thead><tbody>${o}</tbody></table>`:'<p style="color:var(--vscode-descriptionForeground);">No test cycles in this plan.</p>'}
  </div>
...
</html>`}function Xe(i,e){let t=[];if(t.push(`<div class="summary-item"><div class="summary-label">Cycle ID</div><div class="summary-value">${i.id}</div></div>`),t.push(`<div class="summary-item"><div class="summary-label">Plan ID</div><div class="summary-value">${i.test_plan_id}</div></div>`),t.push(`<div class="summary-item"><div class="summary-label">Status</div><div class="summary-value">${H(i.status)}</div></div>`),i.start_date&&t.push(`<div class="summary-item"><div class="summary-label">Start Date</div><div class="summary-value">${new Date(i.start_date).toLocaleDateString()}</div></div>`),i.end_date&&t.push(`<div class="summary-item"><div class="summary-label">End Date</div><div class="summary-value">${new Date(i.end_date).toLocaleDateString()}</div></div>`),i.start_date&&i.end_date){let a=Math.ceil((new Date(i.end_date).getTime()-new Date(i.start_date).getTime())/864e5);t.push(`<div class="summary-item"><div class="summary-label">Duration</div><div class="summary-value">${a} day${a!==1?"s":""}</div></div>`)}t.push(`<div class="summary-item"><div class="summary-label">Total Runs</div><div class="summary-value">${e.length}</div></div>`);let s=Te(e);return`<div class="summary-card">
    <div style="font-size:13px;font-weight:600;color:var(--vscode-descriptionForeground);">SUMMARY</div>
    <div class="summary-grid">${t.join(`
`)}${s}</div>
  </div>`}function Pe(i,e,t,s){let a=pe(i,e),n=s.map(c=>`<tr style="cursor:pointer;" onclick="openRun(${c.id})">
        <td>${E(c.name)}</td>
        <td>${H(c.status)}</td>
        <td>${c.assigned_to??"\u2014"}</td>
        <td>${c.execution_date?new Date(c.execution_date).toLocaleDateString():"\u2014"}</td>
      </tr>`).join(""),o={};try{o=JSON.parse(t.fields_json||"{}")}catch{}let r=U(o.description??t.description??""),d=Xe(t,s);return`<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8" />${me(i.cspSource)}<script src="${a}"></script><style>${ue}</style></head>
<body>
  ${ve()}
  <div style="display:flex;align-items:center;gap:10px;">
    <div class="readonly-field"><h1>${E(t.name)}</h1></div>
...
</html>`}var Se=["NOT_RUN","PASSED","FAILED","BLOCKED","INPROGRESS"];function et(i){if(i.length===0)return"";let e={PASSED:0,FAILED:0,BLOCKED:0,INPROGRESS:0,NOT_RUN:0};for(let t of i){let s=t.status?.toUpperCase()??"NOT_RUN";e[s]=(e[s]??0)+1}return $e(e,i.length,"Steps")}function tt(i,e,t){let s=e.documentKey??"",a=e.duration,n=e.testGroup??"",o=e.priority??"",r=[];if(r.push(`<div class="summary-item"><div class="summary-label">Run ID</div><div class="summary-value">${i.id}</div></div>`),s&&r.push(`<div class="summary-item"><div class="summary-label">Document Key</div><div class="summary-value">${E(s)}</div></div>`),i.test_case_id&&r.push(`<div class="summary-item"><div class="summary-label">Test Case</div><div class="summary-value">${i.test_case_id}</div></div>`),r.push(`<div class="summary-item"><div class="summary-label">Status</div><div class="summary-value">${H(i.status)}</div></div>`),i.assigned_to&&r.push(`<div class="summary-item"><div class="summary-label">Assigned To</div><div class="summary-value">${i.assigned_to}</div></div>`),i.execution_date&&r.push(`<div class="summary-item"><div class="summary-label">Execution Date</div><div class="summary-value">${new Date(i.execution_date).toLocaleDateString()}</div></div>`),a!=null&&a>0){let c=Math.round(a/6e4);r.push(`<div class="summary-item"><div class="summary-label">Duration</div><div class="summary-value">${c>0?c+" min":"<1 min"}</div></div>`)}n&&r.push(`<div class="summary-item"><div class="summary-label">Test Group</div><div class="summary-value">${E(n)}</div></div>`),o&&r.push(`<div class="summary-item"><div class="summary-label">Priority</div><div class="summary-value">${E(o)}</div></div>`);let d=et(t);return`<div class="summary-card">
    <div style="font-size:13px;font-weight:600;color:var(--vscode-descriptionForeground);">SUMMARY</div>
...
`)}
  ...
  </div>`}function Ce(i,e,t){let s=pe(i,e),a={};try{a=JSON.parse(t.fields_json||"{}")}catch{}let n=U(a.description??""),o=(a.testRunSteps??[]).map(m=>({...m,action:U(m.action||""),expectedResult:U(m.expectedResult||""),result:U(m.result||"")})),r=U(a.actualResults??t.actual_results??""),d=tt(t,a,o),c=Se.map(m=>`<option value="${m}" ${m===t.status?"selected":""}>${m}</option>`).join(""),p=o.map((m,v)=>`
...
<html lang="en">
<head><meta charset="UTF-8" />${me(i.cspSource)}<script src="${s}"></script><style>${ue}</style></head>
<body>
  ${ve()}
  <div style="display:flex;align-items:center;gap:10px;">
    <h1>${E(t.name||"Run "+t.id)}</h1>
    <div class="readonly-field"><vscode-tag>${E(t.status)}</vscode-tag></div>
    ${o.length===0?`<div class="editable-field"><select class="field-select" id="f_testRunStatus" data-field="testRunStatus">${c}</select></div>`:`<div class="editable-field"><span style="font-size:12px;color:var(--vscode-descriptionForeground);"><vscode-tag>${E(t.status)}</vscode-tag> (derived from steps)</span></div>`}
  </div>
  ...
  ${o.length>0?`
  <div class="section">
    <h2>Test Steps (${o.length})</h2>
    <div class="readonly-field">${p}</div>
    <div class="editable-field">${g}</div>
  </div>
  `:""}

  ${!n&&o.length===0&&!r?'<p style="color:var(--vscode-descriptionForeground);text-align:center;padding:30px 0;">No additional details available for this test run.</p>':""}

  <script>
    const originalSteps = ${JSON.stringify(o)};
...
</html>`}var B,I,W,G,j;async function st(i){let e=l.window.createOutputChannel("Jama Editor");e.appendLine("Jama Editor extension activating...");let t=new J;B=new X,i.subscriptions.push(B),I=new ee(t,i),W=new te(t,I);let s=l.window.createTreeView("jamaProjects",{treeDataProvider:W,showCollapseAll:!0});i.subscriptions.push(s),G=new ae(t,I);let a=l.window.createTreeView("jamaTestRunner",{treeDataProvider:G,showCollapseAll:!0});i.subscriptions.push(a),j=new ne(t,i.extensionUri),i.subscriptions.push(j);let n=new Z(i.extensionUri);if(i.subscriptions.push(l.window.registerWebviewViewProvider(Z.viewType,n)),i.subscriptions.push(l.commands.registerCommand("jamaEditor.refreshTree",()=>{W?.refresh()}),l.commands.registerCommand("jamaEditor.selectProject",async()=>{await I?.pickProject()}),l.commands.registerCommand("jamaEditor.refreshTestRunner",()=>{G?.refresh()}),l.commands.registerCommand("jamaEditor.openTestDetail",async(r,d)=>{let c=new z,p=d.id,g=d.name||`${r} ${p}`,m=r==="plan"?"Test Plan":r==="cycle"?"Test Cycle":"Test Run",v=l.window.createWebviewPanel("jamaTestDetail",`${m}: ${g}`,l.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[l.Uri.joinPath(i.extensionUri,"out","webview")]});async function w(b){let x=b??d;if(r==="plan"){let h=x,y=[];try{y=await t.getTestCycles(h.id,!0)}catch{}let D=[];try{D=(await Promise.all(y.map(R=>t.getTestRuns(R.id,!0).catch(()=>[])))).flat()}catch{}v.webview.html=Ie(v.webview,i.extensionUri,h,y,D)}else if(r==="cycle"){let h=x,y=[];try{y=await t.getTestRuns(h.id,!0)}catch{}v.webview.html=Pe(v.webview,i.extensionUri,h,y)}else v.webview.html=Ce(v.webview,i.extensionUri,x)}await w(),v.webview.onDidReceiveMessage(async b=>{try{if(b.type==="edit"){if(r==="plan")try{await c.acquireTestPlanLock(p)}catch{}v.webview.postMessage({type:"setEditing",editing:!0,message:"Editing..."})}else if(b.type==="cancel"){if(r==="plan")try{await c.releaseTestPlanLock(p)}catch{}v.webview.postMessage({type:"setEditing",editing:!1,message:"Cancelled"})}else if(b.type==="openCycle"&&b.cycleId)try{let x=d,y=(await t.getTestCycles(x.id,!0)).find(D=>D.id===b.cycleId);y&&l.commands.executeCommand("jamaEditor.openTestDetail","cycle",y)}catch{}else if(b.type==="openRun"&&b.runId)try{let x=d,y=(await t.getTestRuns(x.id,!0)).find(D=>D.id===b.runId);y&&l.commands.executeCommand("jamaEditor.openTestDetail","run",y)}catch{}else if(b.type==="push"){let x=b.fields;try{let h;r==="plan"?h=await c.pushTestPlan(p,x):r==="cycle"?h=await c.pushTestCycle(p,x):h=await c.pushTestRun(p,x);let y=h.status==="no_changes"?"No changes detected":`Pushed to Jama (v${h.version})`;if(r==="plan")try{await c.releaseTestPlanLock(p)}catch{}h.status!=="no_changes"&&h.item?await w(h.item):v.webview.postMessage({type:"pushResult",success:!0,message:y}),G?.refresh(),l.window.showInformationMessage(`${m} ${g}: ${y}`)}catch(h){let y=h instanceof Error?h.message:String(h);v.webview.postMessage({type:"error",message:y}),l.window.showErrorMessage(`Push failed: ${y}`)}}}catch(x){let h=x instanceof Error?x.message:String(x);v.webview.postMessage({type:"error",message:h})}}),v.onDidDispose(async()=>{if(r==="plan")try{await c.releaseTestPlanLock(p)}catch{}})}),l.commands.registerCommand("jamaEditor.openItem",async(r,d,c,p)=>{if(typeof r=="object"&&r!==null&&"itemId"in r){let g=r;await j?.openItem(g.itemId,g.projectId,g.label||"",g.documentKey??"")}else await j?.openItem(r,d,c,p)}),l.commands.registerCommand("jamaEditor.syncProject",async r=>{let d=r?.projectId??I?.selectedId;if(!d){l.window.showWarningMessage("No project selected.");return}try{l.window.withProgress({location:l.ProgressLocation.Notification,title:"Syncing project...",cancellable:!1},async c=>{c.report({message:"Starting sync..."}),await t.syncProject(d),c.report({message:"Sync complete, refreshing tree..."}),W?.refresh(),G?.refresh()})}catch(c){let p=c instanceof Error?c.message:String(c);l.window.showErrorMessage(`Sync failed: ${p}`)}}),l.commands.registerCommand("jamaEditor.createItem",async r=>{let d=r?.projectId??I?.selectedId;if(!d||!r?.itemId)return;let c=await l.window.showInputBox({prompt:"Enter name for the new item",placeHolder:"New Item"});if(c)try{let p=await t.getItemTypes(),g=await l.window.showQuickPick(p.map(m=>({label:m.display,id:m.id})),{placeHolder:"Select item type"});if(!g)return;await t.createItem(d,g.id,r.itemId,{name:c}),l.window.showInformationMessage(`Item "${c}" created.`),W?.refresh()}catch(p){let g=p instanceof Error?p.message:String(p);l.window.showErrorMessage(`Create failed: ${g}`)}}),l.commands.registerCommand("jamaEditor.startBackend",async()=>{await B?.start()?(l.window.showInformationMessage("Jama backend started."),await I?.init()):l.window.showErrorMessage("Failed to start Jama backend. Check Output > Jama Backend.")}),l.commands.registerCommand("jamaEditor.stopBackend",async()=>{await B?.stop(),l.window.showInformationMessage("Jama backend stopped.")}),l.commands.registerCommand("jamaEditor.searchItems",async()=>{let r=await l.window.showInputBox({prompt:"Search Jama items",placeHolder:"Enter search query..."});if(r)try{let d=await t.search(r);if(d.length===0){l.window.showInformationMessage("No results found.");return}let c=await l.window.showQuickPick(d.map(p=>({label:`${p.document_key} \u2014 ${p.name}`,description:p.doc_type,detail:p.snippet,itemId:p.entity_id,projectId:p.project_id,name:p.name,documentKey:p.document_key})),{placeHolder:`${d.length} results for "${r}"`});c&&await j?.openItem(c.itemId,c.projectId,c.name,c.documentKey)}catch(d){let c=d instanceof Error?d.message:String(d);l.window.showErrorMessage(`Search failed: ${c}`)}}),l.commands.registerCommand("jamaEditor.clearAttachmentCache",async()=>{try{let r=j?.getEditorApi();if(!r){l.window.showErrorMessage("Editor backend not available.");return}let d=await r.clearAttachmentCache();l.window.showInformationMessage(`Attachment cache cleared: ${d.files_deleted} files, ${(d.bytes_freed/1024/1024).toFixed(1)} MB freed.`)}catch(r){let d=r instanceof Error?r.message:String(r);l.window.showErrorMessage(`Clear cache failed: ${d}`)}}),l.commands.registerCommand("jamaEditor.incrementalSync",async()=>{let r=I?.selectedId;if(!r){l.window.showWarningMessage("No project selected. Use 'Jama: Select Project' first.");return}try{await l.window.withProgress({location:l.ProgressLocation.Notification,title:"Jama: Incremental sync...",cancellable:!1},async d=>{d.report({message:"Syncing changed items..."}),await t.incrementalSync(r),d.report({message:"Done \u2014 refreshing tree..."}),W?.refresh(),G?.refresh(),l.window.showInformationMessage("Incremental sync complete.")})}catch(d){let c=d instanceof Error?d.message:String(d);l.window.showErrorMessage(`Incremental sync failed: ${c}`)}}),l.commands.registerCommand("jamaEditor.clearImageCache",async()=>{try{let r=j?.getEditorApi();if(!r){l.window.showErrorMessage("Editor backend not available.");return}let d=await r.clearImageCache();l.window.showInformationMessage(`Image cache cleared: ${d.files_deleted} files, ${(d.bytes_freed/1024/1024).toFixed(1)} MB freed.`)}catch(r){let d=r instanceof Error?r.message:String(r);l.window.showErrorMessage(`Clear image cache failed: ${d}`)}}),l.commands.registerCommand("jamaEditor.uploadAttachment",async r=>{try{let d=j?.getEditorApi();if(!d){l.window.showErrorMessage("Editor backend not available.");return}if(!r){let m=await l.window.showInputBox({prompt:"Enter Jama item ID to attach file to",placeHolder:"e.g. 7598533"});if(!m)return;if(r=parseInt(m,10),isNaN(r)){l.window.showErrorMessage("Invalid item ID.");return}}let c=await l.window.showOpenDialog({canSelectMany:!1,openLabel:"Upload to Jama",filters:{Images:["png","jpg","jpeg","gif","bmp","webp","svg"],Reports:["pdf","xlsx","docx","csv","html"],"All Files":["*"]}});if(!c||c.length===0)return;let p=c[0].fsPath,g=p.split(/[\\/]/).pop()||"attachment";await l.window.withProgress({location:l.ProgressLocation.Notification,title:`Uploading ${g}...`,cancellable:!1},async m=>{m.report({message:"Uploading to Jama..."});let v=await d.uploadItemAttachment(r,p,g);l.window.showInformationMessage(`Uploaded "${g}" to item ${r} (attachment ID: ${v.attachment_id})`)})}catch(d){let c=d instanceof Error?d.message:String(d);l.window.showErrorMessage(`Upload failed: ${c}`)}}),l.commands.registerCommand("jamaEditor.openSettings",async()=>{await l.commands.executeCommand("jamaSettingsView.focus")}),l.commands.registerCommand("jamaEditor.manageProjectDbs",()=>{V.show(i.extensionUri)})),l.workspace.getConfiguration("jamaEditor").get("autoStartBackend",!0))if(await B.start()){e.appendLine("Backend ready. Loading project tree..."),await I.init(),I.selectedId&&e.appendLine(`Restored last project: ${I.selectedName} (${I.selectedId})`);try{let d=j?.getEditorApi();if(d){let c=await d.getDirtyItems();if(c.count>0){let g=c.items.filter(v=>v.lock_held);if(g.length>0){let v=g.map(b=>b.item_id).join(", ");if(e.appendLine(`Stale locks detected: items ${v}`),await l.window.showWarningMessage(`${g.length} item(s) have stale locks from a previous session (${v}). Release them?`,"Release All","Ignore")==="Release All")for(let b of g)try{await d.releaseLock(b.item_id),e.appendLine(`Released stale lock: item ${b.item_id}`)}catch{e.appendLine(`Failed to release lock: item ${b.item_id}`)}}let m=await d.getPendingUploads();m.count>0&&(e.appendLine(`${m.count} pending upload(s) found from previous session.`),await l.window.showInformationMessage(`${m.count} attachment upload(s) were interrupted. Retry?`,"Retry","Dismiss")==="Retry"&&(await d.retryPendingUploads(),e.appendLine("Pending uploads retried.")))}}}catch{e.appendLine("Stale lock check skipped (editor backend may still be initializing).")}B.offerServiceInstall();try{(await B.apiClient.getCredentialStatus()).configured||await l.window.showWarningMessage("Jama API credentials are not configured. Open Settings to set them up?","Open Settings")==="Open Settings"&&l.commands.executeCommand("jamaEditor.openSettings")}catch{e.appendLine("Onboarding: credential check skipped (backend may still be initializing).")}}else e.appendLine("Backend failed to start. Use 'Jama: Start Backend' command.");e.appendLine("Jama Editor extension activated.")}function at(){B?.dispose(),j?.dispose()}0&&(module.exports={activate,deactivate});
