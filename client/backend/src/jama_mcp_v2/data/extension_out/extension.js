...
`),Q=new Set(["name","description"]),O=c.filter(S=>!Q.has(S.name)).map(S=>Ge(S,a[S.name],m)),re=O.length,De=re>0?O.join(`
...
`)}${a}</div>
      ...
      </tr>`).join(""),c={};try{c=JSON.parse(t.fields_json||"{}")}catch{}let m=q(c.description??t.description??""),u=Xe(t,s,a??[]);return`<!DOCTYPE html>
<html lang="en">
...
`)}${s}</div>
      ...
      </tr>`).join(""),r={};try{r=JSON.parse(t.fields_json||"{}")}catch{}let c=q(r.description??t.description??""),m=et(t,s);return`<!DOCTYPE html>
<html lang="en">
...
`)}
  ...
  </div>`}function Ce(i,e,t){let s=me(i,e),a={};try{a=JSON.parse(t.fields_json||"{}")}catch{}let n=q(a.description??""),r=(a.testRunSteps??[]).map(d=>({...d,action:q(d.action||""),expectedResult:q(d.expectedResult||""),result:q(d.result||"")})),c=q(a.actualResults??t.actual_results??""),m=st(t,a,r),u=Se.map(d=>`<option value="${d}" ${d===t.status?"selected":""}>${d}</option>`).join(""),o=r.map((d,g)=>`
...
<body>
  ...
  ${t.planned_results?`<div class="section"><h2>Planned Results</h2><div class="desc">${q(t.planned_results)}</div></div>`:""}
