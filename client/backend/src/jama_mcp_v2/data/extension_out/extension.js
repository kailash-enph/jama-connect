...
`):"",Re=n.length,Me=g.length,Ae=Ke(g,o),Le=g.length>0?`<p style="color:var(--vscode-descriptionForeground);font-size:12px;margin-bottom:8px;">
         Use the Versions &amp; Drafts dropdown in the toolbar (in Edit mode) to view a specific version.
       </p>
       <ul style="list-style:none;padding:0;margin:0;">
         ${g.slice().sort((S,oe)=>oe.version_num-S.version_num).map(S=>`<li style="padding:6px 0;border-bottom:1px solid var(--vscode-panel-border);font-size:12px;">
...
<div class="page-content">
  ...
  <div class="meta-bar">
    ...
    <span class="meta-gid" title="Global ID">${E}</span>
  </div>
  ...
  <script nonce="${l}">
    ...
    if (window.JamaTipTap && document.getElementById('descriptionEditor')) {
      const descContent = ${JSON.stringify(y)};
...
</html>`}function We(){let i="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",e="";for(let t=0;t<32;t++)e+=i.charAt(Math.floor(Math.random()*i.length));return e}function $(i){return i==null?"":String(i).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function U(i){return i==null?"":String(i).replace(/"/g,"&quot;").replace(/&/g,"&amp;")}function Ge(i,e,t){let s=$(i.label),a=U(i.name),n=i.required?' <span class="required">*</span>':"",r=i.readOnly?" field-readonly":"",c=i.readOnly?" disabled":"",m=i.readOnly?"":` data-track-field="${a}"`,g=e!=null?String(e):"",o=`dynField_${i.name}`,l;switch(i.fieldType){case"PICK_LIST":{let u=(i.pickList!=null?t[i.pickList]??[]:[]).filter(h=>h.active!==!1).map(h=>{let b=U(String(h.id)),y=$(h.name),I=String(h.id)===g?" selected":"";return`<option value="${b}"${I}>${y}</option>`}).join(`
`);l=`<select id="${o}" data-field-name="${a}"${c} data-track-field="${a}">
        <option value="">(none)</option>
        ${u}
      </select>`;break}case"MULTI_SELECT":{let d=i.pickList!=null?t[i.pickList]??[]:[],u=Array.isArray(e)?e.map(String):[],h=d.filter(b=>b.active!==!1).map(b=>{let y=U(String(b.id)),I=$(b.name),w=u.includes(String(b.id))?" selected":"";return`<option value="${y}"${w}>${I}</option>`}).join(`
`);l=`<select id="${o}" data-field-name="${a}" multiple${c} data-track-field="${a}">
        ${h}
      </select>`;break}case"BOOLEAN":l=`<input type="checkbox" id="${o}" data-field-name="${a}"${e?" checked":""}${c} data-track-field="${a}" />`;break;case"INTEGER":case"FLOAT":l=`<input type="number" id="${o}" data-field-name="${a}" value="${U(g)}"${c}${m}${i.fieldType==="INTEGER"?' step="1"':' step="any"'} />`;break;case"DATE":l=`<input type="date" id="${o}" data-field-name="${a}" value="${U(g)}"${c}${m} />`;break;case"URL":l=`<input type="url" id="${o}" data-field-name="${a}" value="${U(g)}"${c}${m} />`;break;case"RICHTEXT":case"DOCUMENT":l=`<vscode-text-area id="${o}" data-field-name="${a}" value="${U(g)}" rows="5"${c}${m}></vscode-text-area>`;break;case"TEXT":default:{let d=i.maxLength?` maxlength="${i.maxLength}"`:"";l=`<vscode-text-field id="${o}" data-field-name="${a}" value="${U(g)}"${c}${d}${m}></vscode-text-field>`;break}}return`<div class="form-group">
    <label>${s}${n}</label>
    <div class="${r}">${l}</div>
...
`;function he(){return`<div class="toolbar">
  ...
  </div>`}function Ee(i){if(i.length===0)return"";let e={PASSED:0,FAILED:0,BLOCKED:0,INPROGRESS:0,NOT_RUN:0};for(let t of i){let s=t.status?.toUpperCase()??"NOT_RUN";e[s]=(e[s]??0)+1}return $e(e,i.length,"Runs")}function $e(i,e,t){let s=c=>e>0?Math.round(c/e*100):0,a={PASSED:"#22c55e",FAILED:"#ef4444",BLOCKED:"#eab308",INPROGRESS:"#3b82f6",NOT_RUN:"#6b7280"},n=Object.entries(i).filter(([,c])=>c>0).map(([c,m])=>`<span class="step-count"><span class="dot" style="background:${a[c]??"#6b7280"};"></span>${m} ${c}</span>`).join(""),r=Object.entries(i).filter(([,c])=>c>0).map(([c,m])=>`<div class="seg" style="width:${s(m)}%;background:${a[c]??"#6b7280"};"></div>`).join("");return`
    <div class="summary-item" style="grid-column: 1 / -1;">
      <div class="summary-label">${t} Progress (${e})</div>
      <div class="progress-bar">${r}</div>
      <div class="step-summary">${n}</div>
    </div>`}function Xe(i,e,t){let s=[];s.push(`<div class="summary-item"><div class="summary-label">Plan ID</div><div class="summary-value">${i.id}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Project</div><div class="summary-value">${i.project_id}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Status</div><div class="summary-value">${K(i.status)}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Cycles</div><div class="summary-value">${e.length}</div></div>`),s.push(`<div class="summary-item"><div class="summary-label">Total Runs</div><div class="summary-value">${t.length}</div></div>`);let a=Ee(t);return`<div class="summary-card">
...
`)}${a}</div>
      ...
      </tr>`).join(""),c={};try{c=JSON.parse(t.fields_json||"{}")}catch{}let m=F(c.description??t.description??""),g=Xe(t,s,a??[]);return`<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8" />${ue(i.cspSource)}<script src="${n}"></script><style>${ge}</style></head>
<body>
  ${he()}
  <div style="display:flex;align-items:center;gap:10px;">
    <div class="readonly-field"><h1>${T(t.name)}</h1></div>
    <div class="editable-field"><input class="field-input" id="f_name" value="${T(t.name)}" data-field="name" /></div>
    <vscode-tag>${T(t.status)}</vscode-tag>
  </div>
  ...
  <div class="section">
    ...
    <div class="editable-field"><textarea class="field-textarea" id="f_description" data-field="description">${T(typeof m=="string"?m.replace(/<[^>]*>/g,""):"")}</textarea></div>
  </div>
...
</html>`}function et(i,e){let t=[];if(t.push(`<div class="summary-item"><div class="summary-label">Cycle ID</div><div class="summary-value">${i.id}</div></div>`),t.push(`<div class="summary-item"><div class="summary-label">Plan ID</div><div class="summary-value">${i.test_plan_id}</div></div>`),t.push(`<div class="summary-item"><div class="summary-label">Status</div><div class="summary-value">${K(i.status)}</div></div>`),i.start_date&&t.push(`<div class="summary-item"><div class="summary-label">Start Date</div><div class="summary-value">${new Date(i.start_date).toLocaleDateString()}</div></div>`),i.end_date&&t.push(`<div class="summary-item"><div class="summary-label">End Date</div><div class="summary-value">${new Date(i.end_date).toLocaleDateString()}</div></div>`),i.start_date&&i.end_date){let a=Math.ceil((new Date(i.end_date).getTime()-new Date(i.start_date).getTime())/864e5);t.push(`<div class="summary-item"><div class="summary-label">Duration</div><div class="summary-value">${a} day${a!==1?"s":""}</div></div>`)}t.push(`<div class="summary-item"><div class="summary-label">Total Runs</div><div class="summary-value">${e.length}</div></div>`);let s=Ee(e);return`<div class="summary-card">
    <div style="font-size:13px;font-weight:600;color:var(--vscode-descriptionForeground);">SUMMARY</div>
    <div class="summary-grid">${t.join(`
`)}${s}</div>
  </div>`}function Pe(i,e,t,s){let a=me(i,e),n=s.map(g=>`<tr style="cursor:pointer;" onclick="openRun(${g.id})">
        <td>${T(g.name)}</td>
        <td>${K(g.status)}</td>
        <td>${g.assigned_to??"\u2014"}</td>
        <td>${g.execution_date?new Date(g.execution_date).toLocaleDateString():"\u2014"}</td>
...
<body>
  ...
  <div class="section">
    ...
    <div class="editable-field"><textarea class="field-textarea" id="f_description" data-field="description">${T(typeof c=="string"?c.replace(/<[^>]*>/g,""):"")}</textarea></div>
  </div>
...
</html>`}var Se=["NOT_RUN","PASSED","FAILED","BLOCKED","INPROGRESS"];function tt(i){if(i.length===0)return"";let e={PASSED:0,FAILED:0,BLOCKED:0,INPROGRESS:0,NOT_RUN:0};for(let t of i){let s=t.status?.toUpperCase()??"NOT_RUN";e[s]=(e[s]??0)+1}return $e(e,i.length,"Steps")}function st(i,e,t){let s=e.documentKey??"",a=e.duration,n=e.testGroup??"",r=e.priority??"",c=[];if(c.push(`<div class="summary-item"><div class="summary-label">Run ID</div><div class="summary-value">${i.id}</div></div>`),s&&c.push(`<div class="summary-item"><div class="summary-label">Document Key</div><div class="summary-value">${T(s)}</div></div>`),i.test_case_id&&c.push(`<div class="summary-item"><div class="summary-label">Test Case</div><div class="summary-value">${i.test_case_id}</div></div>`),c.push(`<div class="summary-item"><div class="summary-label">Status</div><div class="summary-value">${K(i.status)}</div></div>`),i.assigned_to&&c.push(`<div class="summary-item"><div class="summary-label">Assigned To</div><div class="summary-value">${i.assigned_to}</div></div>`),i.execution_date&&c.push(`<div class="summary-item"><div class="summary-label">Execution Date</div><div class="summary-value">${new Date(i.execution_date).toLocaleDateString()}</div></div>`),a!=null&&a>0){let g=Math.round(a/6e4);c.push(`<div class="summary-item"><div class="summary-label">Duration</div><div class="summary-value">${g>0?g+" min":"<1 min"}</div></div>`)}n&&c.push(`<div class="summary-item"><div class="summary-label">Test Group</div><div class="summary-value">${T(n)}</div></div>`),r&&c.push(`<div class="summary-item"><div class="summary-label">Priority</div><div class="summary-value">${T(r)}</div></div>`);let m=tt(t);return`<div class="summary-card">
    <div style="font-size:13px;font-weight:600;color:var(--vscode-descriptionForeground);">SUMMARY</div>
...
`)}
    ...
    <div style="border:1px solid var(--vscode-panel-border);border-radius:6px;margin-bottom:10px;overflow:hidden;">
      ...
      <div style="padding:12px;font-size:13px;">
        ...
        ${d.notes?`<div><div style="font-size:11px;font-weight:600;color:var(--vscode-descriptionForeground);margin-bottom:2px;">Notes</div><div style="font-size:12px;color:var(--vscode-descriptionForeground);">${T(d.notes)}</div></div>`:""}
      </div>
    </div>`).join(""),l=r.map((d,u)=>`
    <div class="step-card" data-step-index="${u}" style="border:1px solid var(--vscode-panel-border);border-radius:6px;margin-bottom:10px;overflow:hidden;">
      <div style="display:flex;justify-content:space-between;align-items:center;background:var(--vscode-sideBar-background);padding:8px 12px;font-size:12px;">
        <b>Step ${u+1}</b>
        <select class="field-select step-status" data-step="${u}" data-step-field="status">${Se.map(h=>`<option value="${h}" ${h===d.status?"selected":""}>${h}</option>`).join("")}</select>
      </div>
      <div style="padding:12px;font-size:13px;">
        ${d.action?`<div style="margin-bottom:8px;"><div style="font-size:11px;font-weight:600;color:var(--vscode-descriptionForeground);margin-bottom:2px;">Action</div><div>${d.action}</div></div>`:""}
        ${d.expectedResult?`<div style="margin-bottom:8px;"><div style="font-size:11px;font-weight:600;color:var(--vscode-descriptionForeground);margin-bottom:2px;">Expected Result</div><div>${d.expectedResult}</div></div>`:""}
        <div style="margin-bottom:8px;">
          ...
          <textarea class="field-textarea step-field" data-step="${u}" data-step-field="result" rows="3">${T(d.result||"")}</textarea>
        </div>
        <div>
          <div style="font-size:11px;font-weight:600;color:var(--vscode-descriptionForeground);margin-bottom:2px;">Notes</div>
          <textarea class="field-textarea step-field" data-step="${u}" data-step-field="notes" rows="2">${T(d.notes||"")}</textarea>
        </div>
      </div>
...
<body>
  ${he()}
  <div style="display:flex;align-items:center;gap:10px;">
    <h1>${T(t.name||"Run "+t.id)}</h1>
    <div class="readonly-field"><vscode-tag>${T(t.status)}</vscode-tag></div>
    ${r.length===0?`<div class="editable-field"><select class="field-select" id="f_testRunStatus" data-field="testRunStatus">${g}</select></div>`:`<div class="editable-field"><span style="font-size:12px;color:var(--vscode-descriptionForeground);"><vscode-tag>${T(t.status)}</vscode-tag> (derived from steps)</span></div>`}
  </div>
  ...
  <div class="section">
    ...
    <div class="readonly-field">${c?`<div class="desc">${c}</div>`:'<span style="color:var(--vscode-descriptionForeground);">\u2014</span>'}</div>
    <div class="editable-field"><textarea class="field-textarea" id="f_actualResults" data-field="actualResults" rows="4">${T(typeof c=="string"?c.replace(/<[^>]*>/g,""):"")}</textarea></div>
  </div>
...
</html>`}var z,C,q,B,A;async function at(i){let e=p.window.createOutputChannel("Jama Editor");e.appendLine("Jama Editor extension activating...");let t=new O;z=new te,i.subscriptions.push(z),C=new se(t,i),q=new ae(t,C);let s=p.window.createTreeView("jamaProjects",{treeDataProvider:q,showCollapseAll:!0});i.subscriptions.push(s),B=new ne(t,C);let a=p.window.createTreeView("jamaTestRunner",{treeDataProvider:B,showCollapseAll:!0});i.subscriptions.push(a),A=new ie(t,i.extensionUri),i.subscriptions.push(A);let n=new Z(i.extensionUri);i.subscriptions.push(p.window.registerWebviewViewProvider(Z.viewType,n)),i.subscriptions.push(p.commands.registerCommand("jamaEditor.refreshTree",()=>{q?.refresh()}),p.commands.registerCommand("jamaEditor.expandAll",async()=>{await p.commands.executeCommand("list.expandAll")}),p.commands.registerCommand("jamaEditor.selectProject",async(o,l)=>{if(o!==void 0)try{await t.selectProject(o,l)}catch(d){let u=d instanceof Error?d.message:String(d);p.window.showErrorMessage(`Failed to set active project: ${u}`)}}),p.commands.registerCommand("jamaEditor.refreshTestRunner",()=>{B?.refresh()}),p.commands.registerCommand("jamaEditor.openTestDetail",async(o,l)=>{let d=new W,u=l.id,h=l.name||`${o} ${u}`,b=o==="plan"?"Test Plan":o==="cycle"?"Test Cycle":"Test Run",y=p.window.createWebviewPanel("jamaTestDetail",`${b}: ${h}`,p.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0,localResourceRoots:[p.Uri.joinPath(i.extensionUri,"out","webview")]});async function I(w){let E=w??l;if(o==="plan"){let x=E,k=[];try{k=await t.getTestCycles(x.id,!0)}catch{}let j=[];try{j=(await Promise.all(k.map(N=>t.getTestRuns(N.id,!0).catch(()=>[])))).flat()}catch{}y.webview.html=Ie(y.webview,i.extensionUri,x,k,j)}else if(o==="cycle"){let x=E,k=[];try{k=await t.getTestRuns(x.id,!0)}catch{}y.webview.html=Pe(y.webview,i.extensionUri,x,k)}else y.webview.html=Ce(y.webview,i.extensionUri,E)}await I(),y.webview.onDidReceiveMessage(async w=>{try{if(w.type==="edit"){if(o==="plan")try{await d.acquireTestPlanLock(u)}catch{}y.webview.postMessage({type:"setEditing",editing:!0,message:"Editing..."})}else if(w.type==="cancel"){if(o==="plan")try{await d.releaseTestPlanLock(u)}catch{}y.webview.postMessage({type:"setEditing",editing:!1,message:"Cancelled"})}else if(w.type==="openCycle"&&w.cycleId)try{let E=l,k=(await t.getTestCycles(E.id,!0)).find(j=>j.id===w.cycleId);k&&p.commands.executeCommand("jamaEditor.openTestDetail","cycle",k)}catch{}else if(w.type==="openRun"&&w.runId)try{let E=l,k=(await t.getTestRuns(E.id,!0)).find(j=>j.id===w.runId);k&&p.commands.executeCommand("jamaEditor.openTestDetail","run",k)}catch{}else if(w.type==="push"){let E=w.fields;try{let x;o==="plan"?x=await d.pushTestPlan(u,E):o==="cycle"?x=await d.pushTestCycle(u,E):x=await d.pushTestRun(u,E);let k=x.status==="no_changes"?"No changes detected":`Pushed to Jama (v${x.version})`;if(o==="plan")try{await d.releaseTestPlanLock(u)}catch{}x.status!=="no_changes"&&x.item?await I(x.item):y.webview.postMessage({type:"pushResult",success:!0,message:k}),B?.refresh(),p.window.showInformationMessage(`${b} ${h}: ${k}`)}catch(x){let k=x instanceof Error?x.message:String(x);y.webview.postMessage({type:"error",message:k}),p.window.showErrorMessage(`Push failed: ${k}`)}}}catch(E){let x=E instanceof Error?E.message:String(E);y.webview.postMessage({type:"error",message:x})}}),y.onDidDispose(async()=>{if(o==="plan")try{await d.releaseTestPlanLock(u)}catch{}})}),p.commands.registerCommand("jamaEditor.openItem",async(o,l,d,u)=>{if(typeof o=="object"&&o!==null&&"itemId"in o){let h=o;await A?.openItem(h.itemId,h.projectId,h.label||"",h.documentKey??"")}else await A?.openItem(o,l,d,u)}),p.commands.registerCommand("jamaEditor.syncProject",async o=>{let l=o?.projectId??C?.selectedId;if(!l){p.window.showWarningMessage("No project selected.");return}try{p.window.withProgress({location:p.ProgressLocation.Notification,title:"Syncing project...",cancellable:!1},async d=>{d.report({message:"Starting sync..."}),await t.syncProject(l),d.report({message:"Sync complete, refreshing tree..."}),q?.refresh(),B?.refresh()})}catch(d){let u=d instanceof Error?d.message:String(d);p.window.showErrorMessage(`Sync failed: ${u}`)}}),p.commands.registerCommand("jamaEditor.createItem",async o=>{let l=o?.projectId??C?.selectedId;if(!l||!o?.itemId)return;let d=await p.window.showInputBox({prompt:"Enter name for the new item",placeHolder:"New Item"});if(d)try{let u=await t.getItemTypes(),h=await p.window.showQuickPick(u.map(b=>({label:b.display,id:b.id})),{placeHolder:"Select item type"});if(!h)return;await t.createItem(l,h.id,o.itemId,{name:d}),p.window.showInformationMessage(`Item "${d}" created.`),q?.refresh()}catch(u){let h=u instanceof Error?u.message:String(u);p.window.showErrorMessage(`Create failed: ${h}`)}}),p.commands.registerCommand("jamaEditor.startBackend",async()=>{await z?.start()?(p.window.showInformationMessage("Jama backend started."),await C?.init()):p.window.showErrorMessage("Failed to start Jama backend. Check Output > Jama Backend.")}),p.commands.registerCommand("jamaEditor.stopBackend",async()=>{await z?.stop(),p.window.showInformationMessage("Jama backend stopped.")}),p.commands.registerCommand("jamaEditor.searchItems",async()=>{let o=await p.window.showInputBox({prompt:"Search Jama items",placeHolder:"Enter search query..."});if(o)try{let l=await t.search(o);if(l.length===0){p.window.showInformationMessage("No results found.");return}let d=await p.window.showQuickPick(l.map(u=>({label:`${u.document_key} \u2014 ${u.name}`,description:u.doc_type,detail:u.snippet,itemId:u.entity_id,projectId:u.project_id,name:u.name,documentKey:u.document_key})),{placeHolder:`${l.length} results for "${o}"`});d&&await A?.openItem(d.itemId,d.projectId,d.name,d.documentKey)}catch(l){let d=l instanceof Error?l.message:String(l);p.window.showErrorMessage(`Search failed: ${d}`)}}),p.commands.registerCommand("jamaEditor.clearAttachmentCache",async()=>{try{let o=A?.getEditorApi();if(!o){p.window.showErrorMessage("Editor backend not available.");return}let l=await o.clearAttachmentCache();p.window.showInformationMessage(`Attachment cache cleared: ${l.files_deleted} files, ${(l.bytes_freed/1024/1024).toFixed(1)} MB freed.`)}catch(o){let l=o instanceof Error?o.message:String(o);p.window.showErrorMessage(`Clear cache failed: ${l}`)}}),p.commands.registerCommand("jamaEditor.incrementalSync",async()=>{let o=C?.selectedId;if(!o){p.window.showWarningMessage("No project selected. Use 'Jama: Select Project' first.");return}try{await p.window.withProgress({location:p.ProgressLocation.Notification,title:"Jama: Incremental sync...",cancellable:!1},async l=>{l.report({message:"Syncing changed items..."}),await t.incrementalSync(o),l.report({message:"Done \u2014 refreshing tree..."}),q?.refresh(),B?.refresh(),p.window.showInformationMessage("Incremental sync complete.")})}catch(l){let d=l instanceof Error?l.message:String(l);p.window.showErrorMessage(`Incremental sync failed: ${d}`)}}),p.commands.registerCommand("jamaEditor.clearImageCache",async()=>{try{let o=A?.getEditorApi();if(!o){p.window.showErrorMessage("Editor backend not available.");return}let l=await o.clearImageCache();p.window.showInformationMessage(`Image cache cleared: ${l.files_deleted} files, ${(l.bytes_freed/1024/1024).toFixed(1)} MB freed.`)}catch(o){let l=o instanceof Error?o.message:String(o);p.window.showErrorMessage(`Clear image cache failed: ${l}`)}}),p.commands.registerCommand("jamaEditor.uploadAttachment",async o=>{try{let l=A?.getEditorApi();if(!l){p.window.showErrorMessage("Editor backend not available.");return}if(!o){let b=await p.window.showInputBox({prompt:"Enter Jama item ID to attach file to",placeHolder:"e.g. 7598533"});if(!b)return;if(o=parseInt(b,10),isNaN(o)){p.window.showErrorMessage("Invalid item ID.");return}}let d=await p.window.showOpenDialog({canSelectMany:!1,openLabel:"Upload to Jama",filters:{Images:["png","jpg","jpeg","gif","bmp","webp","svg"],Reports:["pdf","xlsx","docx","csv","html"],"All Files":["*"]}});if(!d||d.length===0)return;let u=d[0].fsPath,h=u.split(/[\\/]/).pop()||"attachment";await p.window.withProgress({location:p.ProgressLocation.Notification,title:`Uploading ${h}...`,cancellable:!1},async b=>{b.report({message:"Uploading to Jama..."});let y=await l.uploadItemAttachment(o,u,h);p.window.showInformationMessage(`Uploaded "${h}" to item ${o} (attachment ID: ${y.attachment_id})`)})}catch(l){let d=l instanceof Error?l.message:String(l);p.window.showErrorMessage(`Upload failed: ${d}`)}}),p.commands.registerCommand("jamaEditor.openSettings",async()=>{await p.commands.executeCommand("jamaSettingsView.focus")}),p.commands.registerCommand("jamaEditor.manageProjectDbs",()=>{G.show(i.extensionUri)}),p.commands.registerCommand("jamaEditor.setActiveProjectById",async(o,l)=>{try{await t.selectProject(Number(o),l||`Project ${o}`)}catch{C?.setProjectById(Number(o),l||`Project ${o}`),B?.refresh()}}));let r=null,c=()=>{r?.(),r=t.subscribeEvents((o,l)=>{if(o==="active_project_changed"){let d=l;d.project_id&&(C?.setProjectById(d.project_id,d.project_name??""),B?.refresh())}else o==="sync_complete"?(q?.refresh(),B?.refresh()):o==="item_updated"&&q?.refresh()}),e.appendLine("SSE event stream connected.")},m=je;if(i.subscriptions.push({dispose:()=>{r?.()}}),p.workspace.getConfiguration("jamaEditor").get("autoStartBackend",!0))if(await z.start()){e.appendLine("Backend ready. Loading project tree..."),await C.init(),C.selectedId&&e.appendLine(`Restored last project: ${C.selectedName} (${C.selectedId})`);try{let l=A?.getEditorApi();if(l){let d=await l.getDirtyItems();if(d.count>0){let h=d.items.filter(y=>y.lock_held);if(h.length>0){let y=h.map(w=>w.item_id).join(", ");if(e.appendLine(`Stale locks detected: items ${y}`),await p.window.showWarningMessage(`${h.length} item(s) have stale locks from a previous session (${y}). Release them?`,"Release All","Ignore")==="Release All")for(let w of h)try{await l.releaseLock(w.item_id),e.appendLine(`Released stale lock: item ${w.item_id}`)}catch{e.appendLine(`Failed to release lock: item ${w.item_id}`)}}let b=await l.getPendingUploads();b.count>0&&(e.appendLine(`${b.count} pending upload(s) found from previous session.`),await p.window.showInformationMessage(`${b.count} attachment upload(s) were interrupted. Retry?`,"Retry","Dismiss")==="Retry"&&(await l.retryPendingUploads(),e.appendLine("Pending uploads retried.")))}}}catch{e.appendLine("Stale lock check skipped (editor backend may still be initializing).")}c(),z.offerServiceInstall();try{(await z.apiClient.getCredentialStatus()).configured||await p.window.showWarningMessage("Jama API credentials are not configured. Open Settings to set them up?","Open Settings")==="Open Settings"&&p.commands.executeCommand("jamaEditor.openSettings")}catch{e.appendLine("Onboarding: credential check skipped (backend may still be initializing).")}}else e.appendLine("Backend failed to start. Use 'Jama: Start Backend' command.");e.appendLine("Jama Editor extension activated.")}function je(){z?.dispose(),A?.dispose()}0&&(module.exports={activate,deactivate});
