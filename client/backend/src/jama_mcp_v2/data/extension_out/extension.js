...
</html>`}};var k=I(require("vscode")),ce="jamaEditor.lastProjectId",te=class{constructor(e,t){this.api=e;this.ctx=t}_onDidChange=new k.EventEmitter;onDidChange=this._onDidChange.event;_selectedId;_selectedName="";_projects=[];get selectedId(){return this._selectedId}get selectedName(){return this._selectedName}get projects(){return this._projects}async init(){try{this._projects=await this.api.getProjects()}catch{this._projects=[]}let e=this.ctx.workspaceState.get(ce);e&&this._projects.some(t=>t.id===e)&&(this._selectedId=e,this._selectedName=this._projects.find(t=>t.id===e)?.name??""),this._onDidChange.fire(this._selectedId)}async pickProject(){if(this._projects.length===0)try{this._projects=await this.api.getProjects()}catch{k.window.showErrorMessage("Failed to load projects.");return}let e=this._projects.filter(s=>!s.is_folder).sort((s,a)=>s.name.localeCompare(a.name)).map(s=>({label:s.name,description:s.project_key,detail:s.synced_at>0?"synced":"not synced",projectId:s.id})),t=await k.window.showQuickPick(e,{placeHolder:"Select a Jama project",matchOnDescription:!0});t&&(this._selectedId=t.projectId,this._selectedName=t.label,await this.ctx.workspaceState.update(ce,this._selectedId),this._onDidChange.fire(this._selectedId))}setProjectById(e,t){this._selectedId=e,this._selectedName=t,this.ctx.workspaceState.update(ce,e),this._onDidChange.fire(e)}async refreshProjects(){try{this._projects=await this.api.getProjects()}catch{}}},se=class{_onDidChangeTreeData=new k.EventEmitter;onDidChangeTreeData=this._onDidChangeTreeData.event;api;selector;treeCache=null;constructor(e,t){this.api=e,this.selector=t,t.onDidChange(()=>{this.treeCache=null,this._onDidChangeTreeData.fire()})}refresh(){this.treeCache=null,this._onDidChangeTreeData.fire()}getTreeItem(e){return e}async getChildren(e){let t=this.selector.selectedId;if(!t){let s=new Y("Select a project...",k.TreeItemCollapsibleState.None);return s.command={command:"jamaEditor.selectProject",title:"Select Project"},s.iconPath=new k.ThemeIcon("folder-opened"),[s]}return e?e.contextValue==="jamaFolder"||e.contextValue==="jamaItem"?this.getItemChildren(t,e.itemId):[]:this.getProjectItems(t)}async getProjectItems(e){try{if(this.treeCache===null&&(this.treeCache=await this.api.getItemTree(e)),this.treeCache.length===0){let t=new Y("No items cached for this project",k.TreeItemCollapsibleState.None);return t.description="Open DB Manager to download or sync",t.iconPath=new k.ThemeIcon("cloud-download"),t.command={command:"jamaEditor.manageProjectDbs",title:"Manage Project Databases"},[t]}return this.treeCache.map(t=>this.nodeToTreeItem(t,e))}catch(t){let s=t instanceof Error?t.message:String(t);return k.window.showErrorMessage(`Failed to load project tree: ${s}`),[]}}getItemChildren(e,t){let s=this.findNode(this.treeCache??[],t);return!s||!s.children?[]:s.children.map(a=>this.nodeToTreeItem(a,e))}findNode(e,t){for(let s of e){if(s.id===t)return s;if(s.children){let a=this.findNode(s.children,t);if(a)return a}}}nodeToTreeItem(e,t){let s=e.has_children&&e.children&&e.children.length>0,a=s?k.TreeItemCollapsibleState.Collapsed:k.TreeItemCollapsibleState.None,n=new Y(e.name,a);return n.projectId=t,n.itemId=e.id,n.documentKey=e.document_key,n.description=`${e.document_key} \xB7 ${e.item_type_display}`,n.tooltip=`${e.section_label} ${e.document_key} \u2014 ${e.name}`,n.contextValue=s?"jamaFolder":"jamaItem",n.iconPath=this.getItemIcon(e.item_type_display),n.command={command:"jamaEditor.openItem",title:"Open Item",arguments:[e.id,t,e.name,e.document_key]},n}getItemIcon(e){let t=e.toLowerCase();return t.includes("requirement")?new k.ThemeIcon("checklist"):t.includes("test")?new k.ThemeIcon("beaker"):t.includes("component")?new k.ThemeIcon("symbol-class"):t.includes("set")||t.includes("folder")?new k.ThemeIcon("folder"):t.includes("text")?new k.ThemeIcon("file-text"):new k.ThemeIcon("symbol-misc")}},Y=class extends k.TreeItem{projectId;itemId;documentKey};var f=I(require("vscode")),ae=class{_onDidChangeTreeData=new f.EventEmitter;onDidChangeTreeData=this._onDidChangeTreeData.event;api;selector;constructor(e,t){this.api=e,this.selector=t,t.onDidChange(()=>{this._onDidChangeTreeData.fire()})}refresh(){this._onDidChangeTreeData.fire()}getTreeItem(e){return e}async getChildren(e){let t=this.selector.selectedId;if(!t){let s=new A("Select a project...",f.TreeItemCollapsibleState.None);return s.command={command:"jamaEditor.selectProject",title:"Select Project"},s.iconPath=new f.ThemeIcon("folder-opened"),[s]}return e?e.contextValue==="jamaTestPlan"?this.getTestCycles(e.testPlanId):e.contextValue==="jamaTestCycle"?this.getTestRuns(e.testCycleId):[]:this.getTestPlans(t)}async getTestPlans(e){try{let t=await this.api.getTestPlans(e,!0);if(t.length===0){let s=new A("No test plans",f.TreeItemCollapsibleState.None);return s.iconPath=new f.ThemeIcon("info"),[s]}return t.map(s=>{let a=new A(s.name,f.TreeItemCollapsibleState.Collapsed);return a.contextValue="jamaTestPlan",a.testPlanId=s.id,a.testPlanData=s,a.description=s.status,a.tooltip=`${s.name}
...
`):"",je=n.length,Re=c.length,Me=Ge(c,p),Ae=c.length>0?`<p style="color:var(--vscode-descriptionForeground);font-size:12px;margin-bottom:8px;">
       ...
       <ul style="list-style:none;padding:0;margin:0;">
         ${c.slice().sort((S,oe)=>oe.version_num-S.version_num).map(S=>`<li style="padding:6px 0;border-bottom:1px solid var(--vscode-panel-border);font-size:12px;">
              <b>v${S.version_num}</b>${S.modified_by?` \u2014 ${T(String(S.modified_by))}`:""}${S.created_date?` <span style="color:var(--vscode-descriptionForeground)">(${T(S.created_date)})</span>`:""}
            </li>`).join("")}
