      ...
      async (type: string, data: JamaTestPlan | JamaTestCycle | JamaTestRun) => {
        ...
        async function renderPanel(freshData?: JamaTestPlan | JamaTestCycle | JamaTestRun) {
          ...
          if (type === "plan") {
            ...
            panel.webview.html = getTestPlanDetailHtml(panel.webview, plan, cycles, allRuns);
          } else if (type === "cycle") {
            const cycle = d as JamaTestCycle;
            let runs: JamaTestRun[] = [];
            try { runs = await api.getTestRuns(cycle.id, true); } catch { /* ignore */ }
            panel.webview.html = getTestCycleDetailHtml(panel.webview, cycle, runs);
          } else {
            panel.webview.html = getTestRunDetailHtml(panel.webview, d as JamaTestRun);
          }
        }
